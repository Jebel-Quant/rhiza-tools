...
Generate a coverage badge endpoint JSON for shields.io. This reads a `coverage.json` file (typically generated by pytest-cov) and creates a JSON file compatible with shields.io's endpoint badge feature.

**Usage:**

```bash
# As plugin
rhiza tools coverage-badge
...
*   `--coverage-json, -c` - Path to coverage.json file. Defaults to config or `_tests/coverage.json`.
*   `--output-dir, -o` - Path to output directory. Defaults to config or `_book/tests`.
*   `--badge-filename, -b` - Badge filename. Defaults to config or `coverage-badge.json`.
*   `--dry-run` - Print what would happen without actually writing files.
