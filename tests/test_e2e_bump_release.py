...
def e2e_project(tmp_path, monkeypatch):
    ...
    # Initialize git repo
    subprocess.run(["git", "init"], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)  # nosec B603
...
"""  # noqa: E501
    ...
    subprocess.run(["git", "commit", "-m", "Initial commit"], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)  # nosec B603
...
class TestBumpOnBranch:
    ...
    def test_bump_on_branch_and_restore(self, e2e_project):
        ...
        subprocess.run(["git", "checkout", "-b", "feature-branch"], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)  # nosec B603 B607
        subprocess.run(["git", "checkout", "-"], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)  # nosec B603 B607
...
class TestInteractiveRelease:
    ...
    def test_release_dry_run_shows_info(self, e2e_project):
        ...
        subprocess.run(["git", "tag", "v0.1.0"], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)  # nosec B603
        ...
        with (
            ...
            patch("rhiza_tools.commands.release.versioning.get_interactive_bump_type", return_value="0.1.0"),
        ):
...
class TestInteractiveReleaseWithBump:
    ...
    def test_release_with_bump_flag_dry_run(self, e2e_project):
        ...
        with (
            patch("rhiza_tools.commands.release.git.run_git_command", side_effect=mock_git),
            patch("rhiza_tools.commands.release.versioning.bump_command") as mock_bump,
        ):
...
class TestNonInteractiveReleaseDryRun:
    ...
    def test_release_dry_run_no_changes(self, e2e_project):
        ...
        with patch("rhiza_tools.commands.release.git.run_git_command", side_effect=mock_git):
...
class TestBumpDirtyWorkingDirectory:
    ...
    def test_bump_dirty_with_allow_dirty_succeeds(self, e2e_project):
        ...
        subprocess.run(["git", "add", "dirty.txt"], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)  # nosec B603 B607
...
class TestSequentialBumpRelease:
    ...
    def test_bump_then_release_dry_run(self, e2e_project):
        ...
        with (
            ...
            patch("rhiza_tools.commands.release.typer.confirm", return_value=True),
            patch("rhiza_tools.commands.release.versioning.bump_command"),
            patch("rhiza_tools.commands.release.versioning.get_interactive_bump_type", return_value="0.1.1"),
        ):
            release_command(dry_run=True)
